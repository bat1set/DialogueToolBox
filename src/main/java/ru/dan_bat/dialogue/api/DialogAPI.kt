package ru.dan_bat.dialogue.api

import net.minecraft.resources.ResourceLocation
import net.minecraft.server.level.ServerPlayer
import ru.dan_bat.dialogue.MainScreen
...
import ru.dan_bat.dialogue.network.ShowDialogListPacket
import ru.dan_bat.dialogue.network.ShowSingleDialogPacket
import ru.dan_bat.dialogue.network.toSerializable
...
object DialogAPI {
    ...
    fun showDialog(player: ServerPlayer, text: String, iconPath: String? = null, config: DialogConfig = DialogConfig()) {
        val iconLocationStr = iconPath?.let { "${MainScreen.MODID}:$it" }
        ShowSingleDialogPacket(text, iconLocationStr, config).send(player)
    }
    ...
    fun showDialog(player: ServerPlayer, setup: DialogBuilder.() -> Unit) {
        ...
        val iconLocationStr = builder.getIconPath()?.let { "${MainScreen.MODID}:$it" }
        ShowSingleDialogPacket(builder.getText(), iconLocationStr, config).send(player)
    }

    @JvmStatic
    fun showDialogs(player: ServerPlayer, dialogs: List<ExtendedDialogData>) {
        ShowDialogListPacket(dialogs.map { it.toSerializable() }).send(player)
    }
